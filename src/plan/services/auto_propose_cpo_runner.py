...
def _build_consequences(
    ...
    # Arredonda a € inteiro: evita "Margem -0 €" para |delta| < 0.5 (round(-0.4)=0).
    delta_eur = round(cost_delta) if cost_delta is not None else None
    ...
    if delta_eur is not None:
        if_accept.append(f"Margem {delta_eur:+d} € vs plano atual")
    if quality_risk:
    ...
    # `target` pode ser uma OF ou um barco (defeito) — frase neutra evita
    # chamar "ordem" a um barco.
    if_reject: list[str] = []
    if if_accept:
        if_reject.append("Mantém o plano atual — sem replaneamento")
        if target:
            if_reject.append(
                f"O replaneamento proposto para {target} não é aplicado"
            )
    ...
    if if_reject:
        ...
        if delta_eur is not None:
            bits.append(f"margem {delta_eur:+d} €")
        if quality_risk:
