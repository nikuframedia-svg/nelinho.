"""
ProdPlan ONE - Report Retention Cleanup
========================================

``ReportRun`` rows accumulate one per generation / delivery. The
retention job prunes runs older than their owning schedule's
...
async def cleanup_expired_runs(
    ...
    """Delete ``ReportRun`` rows past their retention window.

    Retention per run = the owning schedule's ``retention_days``, or
    ``default_retention_days`` for ad-hoc runs. Returns the number of
    runs deleted.
    """
    now = datetime.utcnow()
    ...
    deleted = 0
    for run in runs:
        retention = retention_by_schedule.get(run.schedule_id, default_retention_days)
        ...
        session.add(
            AuditLog(
                ...
                reason=f"retention cleanup ({retention}d)",
            )
        )
    ...
    if deleted:
        ...
        logger.info("report retention cleanup: deleted %d run(s) for tenant %s", deleted, tenant_id)
    return deleted
