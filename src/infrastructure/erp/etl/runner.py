...
(``core.products`` etc.) carry no DB-level unique constraint on their
business keys. Volumes are small (master data <2k; quality imported in
windows) so an indexed lookup per batch is cheap and avoids touching
existing table DDL.

Usage::

    async with EtlRunner(session, tenant_id, source="master_data") as run:
...
class EtlRunner:
    ...
    async def __aenter__(self) -> "EtlRunner":
        ...
        logger.info("etl_run start source=%s tenant=%s", self.result.source, self.tenant_id)
        return self
    ...
    async def upsert(
        ...
        *,
        ...
        Returns ``(inserted, updated)`` and adds the same to the run tally.
        Re-running with identical data yields ``(0, 0)``.
        """
