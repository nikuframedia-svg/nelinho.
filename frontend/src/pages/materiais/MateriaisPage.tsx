 ...
 *   - Entregas  → /v1/supply/purchase-orders (REAL — Q.53.D/J). Degrada com
 *                 honestidade: mirror nunca sincronizado → empty state.
 *   - Fornecedores → /v1/core/suppliers (REAL).
 *
 * Q.53.J — o catálogo mostra a `predicted_stockout_date` (rutura prevista a
 * partir do consumo histórico real do ledger) e a tab Entregas lista as
 * encomendas a fornecedor com ETA e estado de receção.
 */
...
const CATALOG_GRID = '2fr 0.6fr 1fr 1.3fr 0.9fr 0.8fr';

/** Q.53.J — cor da etiqueta de confiança da previsão de rutura. */
const STOCKOUT_CONFIDENCE_LABEL: Record<string, string> = {
  high: 'fiável',
  medium: 'estimada',
  low: 'incerta',
  none: '—',
 ...
 */
function StockoutCell({ material: m }: { material: BomMaterial }): ReactNode {
...
type EntregasFiltro = 'todas' | 'abertas' | 'atrasadas';

const ENTREGAS_GRID = '1.5fr 1.7fr 0.9fr 1fr 1fr 0.9fr';
...
function PurchaseOrderRow({ po }: { po: PurchaseOrder }): ReactNode {
