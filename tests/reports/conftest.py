"""Shared fakes for the reports test suite (Sprint Q.22.D-G).
...
lists. It implements only the surface the reports API / cleanup job
actually use.
"""
...
class FakeSession:
    """Minimal AsyncSession stand-in for the reports API + cleanup job.

    ``add`` assigns the primary key + timestamps that a real flush would
    populate. ``execute`` returns rows by inspecting which table the
    compiled statement targets — good enough for the single-table CRUD
    the reports endpoints issue.
    """
    ...
    def __init__(self):
        ...
        # (e.g. "hr_allocations"). Each value is a list of Row tuples.
