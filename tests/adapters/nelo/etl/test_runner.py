...
Two surfaces:
  * the idempotent ``upsert`` — exercised against a real aiosqlite DB with
    a minimal test model, so the load-existing / insert / update logic
    actually runs;
...
async def test_upsert_composite_key(session):
    """A two-field business key resolves via the tuple_() IN-clause."""
    rows = [
        {"code": "A", "name": "x1", "value": 1},
        {"code": "A", "name": "x2", "value": 2},
    ]
    inserted, _ = await _runner(session).upsert(
        _Widget, rows, key_fields=["code", "name"],
    )
    assert inserted == 2
    # Re-run with one changed value — composite key matches, updates 1.
    _, updated = await _runner(session).upsert(
        _Widget,
        [{"code": "A", "name": "x1", "value": 99}],
